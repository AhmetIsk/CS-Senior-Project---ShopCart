  ...
  const ListProducts = () => {
    userService.getShoppingList(token).then(products => {
      ...
      const mappedItems = products.map(product => {
        return <ProductRow key={product.id} name={product.product.name} quantity={product.quantity} barcode={product.product.barcode} trigger={setRerender} value={rerender}/>
      });
  ...
  return (
    <View style={styles.container}>
      ...
      <DataTable.Header>
          ...
          <DataTable.Title numeric sortDirection='descending'>Quantity</DataTable.Title>
